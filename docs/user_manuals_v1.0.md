...
5.  如需使用商用 OCR，请先在 **“AI 模型设置”** 中选择 OCR Provider（`textin` 或 `glm-ocr`），并填写 API Key 与 Base URL。
    *   选择 `textin` 时，系统会预置 Base URL：`https://api.textin.com/ai/service/v2/recognize/document`，其中 API Key 填 `Secret Code`，OCR Model 填 `App ID`。
...
4.  快速入库同样读取 **“AI 模型设置”** 中的 OCR 商业模型配置（`textin` / `glm-ocr`，仅当前浏览器会话）。
