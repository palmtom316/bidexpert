...
### 2.2 投标资料入库（批量上传）
这是最常用的功能，用于将现有的 PDF、Word 文档导入系统。

**操作步骤：**
1.  **选择文件**：点击“资料文件”下的输入框，选择您电脑上的文件（支持 PDF, Word, Markdown）。支持多选。
2.  **选择资料类型**：
...
3.  **填写工程类别**（可选）：例如“光伏”、“房建”、“市政”。如果不填，系统会自动尝试识别。
4.  **点击“开始入库”**：
    *   系统会显示“正在上传...”。
    *   上传成功后，后台会自动进行 OCR 文字识别、段落拆解和向量化处理。
    *   **注意**：大文件处理需要时间，请耐心等待“处理完成”的提示。
...
4.  **Re-ranking**: (可选) 对融合结果进行重排序 (v3.7 Spec)。
...
### 3.3 版本升级 (v3.7 Specifics)
*   **Schema 变更**：v3.7 引入了 `sensitivity_level` (敏感度) 和 `review_report` 表。请确保运行了最新的 SQL 迁移或初始化脚本。
